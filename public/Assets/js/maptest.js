var mapLoadingWarning = $("#mapLoadingWarning");
...
//initial map is created to display on dashboard
var map;
function initMap() {
  ...
  $.ajax({
    ...
    success: function (response) {
      for (var i = 0; i < response.data.length; i++) {
        ...
        //creating circle on the map with infowindow
      ...
      mapLoadingWarning.hide();
    }
  });

  //for index page eventlistener to click state
  var abbrev = localStorage.getItem("Abbreviation");
  abbrev = abbrev.substring(7)

  for (var i = 0; i < states.length; i++) {
    var lat = states[i].latitude;
    var lng = states[i].longitude;
    if (abbrev === states[i].state) {
      map.setZoom(6);
